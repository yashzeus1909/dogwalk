...
$(document).ready(function () {
    ...
    function bindWalkerEvents() {
        $(document).on("click", ".book-walker-btn", function (e) {
            ...
            console.log(
                "Book walker clicked, redirecting to booking page for walker ID:",
                walkerId,
            );
    ...
    function submitBooking() {
        ...
        if (
            !bookingData.dog_name ||
            !bookingData.dog_size ||
    ...
    function updateBookingStatus(bookingId, newStatus) {
        ...
        if (
            confirm(
                `Are you sure you want to ${statusText[newStatus]} this booking?`,
            )
        ) {
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        showToast(
                            `Booking ${statusText[newStatus]}ed successfully!`,
                            "success",
                        );
    ...
    function loadProfileBookings() {
        ...
        container.html(
            '<div class="text-center py-4"><i class="fas fa-spinner fa-spin"></i> Loading your bookings...</div>',
        );
